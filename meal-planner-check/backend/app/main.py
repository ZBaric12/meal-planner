...
from .auth import get_db, get_current_user, create_access_token, hash_pw, verify_pw
...
def list_meals(from_: str | None = None, to: str | None = None,
    ...
    if from_: q = q.filter(Meal.date >= from_)
    if to:   q = q.filter(Meal.date <= to)
    return q.order_by(Meal.date.asc()).all()

@app.post("/meals", response_model=MealOut, status_code=201)
def create_meal(data: MealCreate, db: Session = Depends(get_db), current=Depends(get_current_user)):
...
def _get_owned_meal(db: Session, meal_id: int, user_id: int) -> Meal:
    ...
    if not meal: raise HTTPException(status_code=404, detail="Nije pronađeno")
    return meal

@app.get("/meals/{meal_id}", response_model=MealOut)
def get_meal(meal_id: int, db: Session = Depends(get_db), current=Depends(get_current_user)):
    return _get_owned_meal(db, meal_id, current.id)

@app.put("/meals/{meal_id}", response_model=MealOut)
def update_meal(meal_id: int, data: MealUpdate, db: Session = Depends(get_db), current=Depends(get_current_user)):
...
def delete_meal(meal_id: int, db: Session = Depends(get_db), current=Depends(get_current_user)):
    meal = _get_owned_meal(db, meal_id, current.id)
